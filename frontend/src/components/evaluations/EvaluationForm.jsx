...
function InputField({ label, name, value, onChange, type = 'text', placeholder = '', step, min, max }) {
  return (
    <label className="block">
      <span className="mb-2 block text-sm font-medium text-slate-600">{label}</span>
      <input
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <SelectField
          label="Epreuve"
          name="idEpreuve"
          value={values.idEpreuve}
          onChange={onChange}
          required
          options={epreuves.map((epreuve) => ({ value: `${epreuve.id}`, label: epreuve.libelle }))}
