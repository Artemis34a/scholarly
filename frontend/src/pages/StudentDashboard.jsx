...
import { formatMontant } from './paiements/paiements.utils'

const JOURS = ['DIMANCHE', 'LUNDI', 'MARDI', 'MERCREDI', 'JEUDI', 'VENDREDI', 'SAMEDI']

function StudentDashboard() {
  const { user } = useAuth()
  const dashboardModule = studentModules[0]
  const featureModules = studentModules.filter((module) => module.id !== 'dashboard')
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      evaluationsService.notes.findAll({ eleveId: user.id, page: 1, limit: 5 }),
      paiementsService.getHistorique(user.id),
      disciplinesService.getHistorique(user.id),
      emploiDuTempsService.getGrilleEleve(user.id),
    ])
      .then(([bulletin, notesResult, paiementsHistorique, disciplineHistorique, slots]) => {
        if (!isMounted) return

        const soldeRestant = paiementsHistorique.parAnnee.reduce((sum, annee) => sum + annee.reste, 0)
        const jourAujourdhui = JOURS[new Date().getDay()]
        const coursAujourdhui = slots.filter((slot) => slot.jour === jourAujourdhui).length

        setStats({
          moyenne: bulletin.moyenneGenerale !== null && bulletin.moyenneGenerale !== undefined
            ? `${bulletin.moyenneGenerale.toFixed(2)}/20`
            : 'Aucune note',
          coursAujourdhui,
          rapports: disciplineHistorique.total,
          solde: formatMontant(soldeRestant),
        })

        setDernieresNotes(notesResult.data.slice(0, 3))
      })
      .finally(() => {
  ...
  return (
    <div className="space-y-6 md:space-y-7">
      <Card className="overflow-hidden border-transparent bg-slate-900 p-1 text-white shadow-[0_36px_110px_-46px_rgba(15,23,42,0.78)]">
        <div className="rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top_right,_rgba(56,189,248,0.34),_transparent_28%),linear-gradient(135deg,_rgba(15,23,42,1),_rgba(30,41,59,0.95))] p-6 md:p-8">
          ...
          <h2 className="mt-4 text-3xl font-semibold md:text-4xl">
            Bienvenue, {user?.nom || user?.username || 'Élève'}
          </h2>
      ...
      <section className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        <SummaryCard title="Moyenne generale" value={loading ? '…' : stats.moyenne} detail="Toutes evaluations" accent={dashboardModule.accent} />
        <SummaryCard title="Cours aujourd'hui" value={loading ? '…' : stats.coursAujourdhui} detail="Emploi du temps" accent={dashboardModule.accent} />
        <SummaryCard title="Rapports disciplinaires" value={loading ? '…' : stats.rapports} detail="Historique complet" accent={dashboardModule.accent} />
        <SummaryCard title="Solde a payer" value={loading ? '…' : stats.solde} detail="Toutes annees" accent={dashboardModule.accent} />
      </section>

      <section className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]">
        <Card
          title="Accès rapide"
          subtitle="Chaque carte ouvre un espace dédié à votre suivi scolaire."
        >
        ...
        <Card title="Dernieres notes" subtitle="Vos evaluations les plus recentes.">
          {loading ? (
            ...
            <div className="rounded-[24px] border border-dashed border-slate-200 bg-slate-50/80 px-4 py-8 text-center">
              <p className="text-sm font-semibold text-slate-700">Aucune note enregistree pour le moment.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {dernieresNotes.map((evaluation) => (
                <div key={evaluation.id} className="rounded-[24px] border border-slate-200/70 bg-slate-50/90 p-4">
