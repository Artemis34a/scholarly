...
import CoursTable from '../../components/cours/CoursTable'
import { classesService } from '../../services/classesService'
import { coursService } from '../../services/coursService'
import { BUTTON_ON_DARK } from '../../components/buttonStyles'
import { applyCoursFilters } from './cours.utils'
...
function CoursPage() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        const result = await coursService.findAll({
  ...
  async function handleDelete(cours) {
    ...
    try {
      ...
      const isLastItemOnPage = coursList.length === 1 && page > 1
      if (isLastItemOnPage) {
  ...
  return (
    <div className="space-y-6 md:space-y-7">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <StatCard label="Cours affiches" value={stats.total} detail="Resultats apres filtres" />
        <StatCard label="Cours actifs" value={stats.actifs} detail="Statut actif uniquement" />
        <StatCard label="Avec enseignant" value={stats.avecEnseignant} detail="Sur cette page" />
      </section>
      ...
      >
        ...
        {loading ? (
          <div className="py-12 text-center text-slate-400">Chargement des cours...</div>
        ) : (
          <>
            <CoursTable
