...
import HistoriqueView from '../../components/disciplines/HistoriqueView'
...
function HistoriquePage() {
  ...
  useEffect(() => {
    ...
    disciplinesService.getHistorique(idEleve)
      .then((data) => {
    ...
    <div className="space-y-6 md:space-y-7">
      <Card className="overflow-hidden border-transparent bg-slate-900 p-1 text-white shadow-[0_36px_110px_-46px_rgba(15,23,42,0.78)]">
        <div className="rounded-[28px] border border-white/10 bg-[radial-gradient(circle_at_top_right,_rgba(56,189,248,0.34),_transparent_28%),linear-gradient(135deg,_rgba(15,23,42,1),_rgba(30,41,59,0.95))] p-6 md:p-8">
          ...
          <h2 className="mt-4 text-3xl font-semibold md:text-4xl">Historique disciplinaire par eleve</h2>
          <p className="mt-4 max-w-3xl text-sm leading-7 text-slate-300 md:text-base">
            Consultez le parcours complet d'un eleve : fautes, comportements positifs et statut des dossiers.
          </p>
        </div>
      </Card>
      ...
      <Card title="Selectionner un eleve" subtitle="Choisissez un eleve pour afficher son historique.">
        {loading ? (
        ...
        <>
          {loadingHistorique || !historique ? (
            ...
            <HistoriqueView
              ...
              title={eleveSelectionne ? `Historique de ${eleveSelectionne.nom} ${eleveSelectionne.prenom}` : 'Historique'}
            />
          )}
        </>
      )}
